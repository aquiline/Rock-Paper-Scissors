...
void youWin()
{
    ...
    glBegin(GL_POLYGON);
        glTexCoord2d(0,0);  glVertex2f(450,250);
        glTexCoord2d(1,0);  glVertex2f(800,250);
        glTexCoord2d(1,1);  glVertex2f(800,550);
        glTexCoord2d(0,1);  glVertex2f(450,550);
    glEnd();
    glDisable(GL_TEXTURE_2D);
    glPopMatrix();
    glFlush();
...
{
	switch (key) {
	case 'z':  userChoice = 1;same = 0;pdx= 0;cdx = 0; glutPostRedisplay(); break;
	case 'x':  userChoice = 2;same = 0;pdx= 0;cdx = 0; glutPostRedisplay(); break;
	case 'c':  userChoice = 3;same = 0;pdx= 0;cdx = 0; glutPostRedisplay(); break;
	}
}
